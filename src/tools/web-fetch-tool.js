...
// Serializes read-modify-write access to the shared 'ollamaFetchedUrls' brain
// key across concurrent tool invocations, mirroring the summarizationInProgress
// lock pattern in hubot-ollama.js. A queue (not a skip-if-locked flag) since
// concurrent fetches must all still complete rather than being dropped.
const brainAccessLocks = new Map();
const withBrainLock = async (key, fn) => {
  const prior = brainAccessLocks.get(key) || Promise.resolve();
  const run = prior.catch(() => {}).then(fn);
  const guarded = run.catch(() => {});
  brainAccessLocks.set(key, guarded);
  try {
    return await run;
  } finally {
    if (brainAccessLocks.get(key) === guarded) {
      brainAccessLocks.delete(key);
    }
  }
};
...
module.exports = (ollama, config, logger) => ({
  ...
  handler: async (args, robot) => {
    ...
    try {
      ...
      // The whole read-modify-write cycle over the shared 'ollamaFetchedUrls'
      // brain key is serialized so concurrent tool invocations (same or
      // different rooms/users) can't interleave and lose each other's updates.
      return await withBrainLock('ollamaFetchedUrls', async () => {
        ...
        // Track fetched URLs (re-read in case another invocation wrote in the meantime)
        const latestFetchedUrls = robot.brain.get('ollamaFetchedUrls');
        if (!Array.isArray(latestFetchedUrls[invocationContextKey])) {
          latestFetchedUrls[invocationContextKey] = [];
        }
        for (const page of pages) {
          if (page.url && !latestFetchedUrls[invocationContextKey].includes(page.url)) {
            latestFetchedUrls[invocationContextKey].push(page.url);
          }
        }
        robot.brain.set('ollamaFetchedUrls', latestFetchedUrls);
